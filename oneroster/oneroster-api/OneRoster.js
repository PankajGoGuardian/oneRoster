...
OneRoster.prototype.makeRosterRequest = async function(pathUrl, urlParams) { 
  ...
  let offset = urlParams?.offset || 0
  // calculating iteration required for the given limit
  let iteration = limit? Math.ceil(limit / DEFAULT_PAGE_SIZE): 0
  // If only one iteration require, changing page size to limit
  if (iteration === 1) {
  ...
  let key
  do {
    let response = await this.buildReqHeaderAndmakeRosterRequest(url, {...urlParams, limit: DEFAULT_PAGE_SIZE, offset})
    // In case of no limit passed, considering total data size as limit
    if (!limit) {
      limit = response.headers['x-total-count']
      // Recalculating iteration in case of no limit
    ...
    let data = body[key]
    // if API throws any error, same will be thrown from here
    if (data === 'error') {
    ...
    iteration--
    // Calculating offset after every API response
    offset += DEFAULT_PAGE_SIZE
    // If next iteration is the last iteration and we have data to fetch 
...
let filterSupportedUrlParams = function(urlParams) {
    const {filter, sort, orderBy, limit, offset, feilds, ext_basic} = urlParams
    ...
    switch (key) {
      ...
      case 'academicSessions': {
        ...
        for (const academicSession of apiData) {
          const modifiedAcademicSession = { ...academicSession }
          modifiedAcademicSession.parentSourcedId =
            academicSession.parent.sourcedId
      ...
      case 'courses': {
        ...
        for (const course of apiData) {
          const modifiedCourseObj = { ...course }
      ...
      case 'enrollments': {
        ...
        for (const enrollment of apiData) {
          const modifiedEnrollmentObj = { ...enrollment }
